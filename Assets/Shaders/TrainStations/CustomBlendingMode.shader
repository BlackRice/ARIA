//#EditorFriendly
//#node10:posx=-216:posy=87:title=DepthBlend:input0=1:input0type=float:
//#node9:posx=-227:posy=3:title=Multiply:input0=1:input0type=float:input0linkindexnode=8:input0linkindexoutput=0:input1=2:input1type=float:
//#node8:posx=-404:posy=-33:title=Multiply:input0=1:input0type=float:input0linkindexnode=5:input0linkindexoutput=0:input1=1:input1type=float:input1linkindexnode=6:input1linkindexoutput=0:
//#node7:posx=-200:posy=-118:title=Color:input0=(0,0,0,1):input0type=Color:
//#node6:posx=-532:posy=24:title=SceneColor:input0=(0,0):input0type=Vector2:
//#node5:posx=-533:posy=-100:title=Texture:title2=Texture1:input0=(0,0):input0type=Vector2:
...
//#masterNode:posx=0:posy=0:title=Master Node:input0linkindexnode=7:input0linkindexoutput=0:input1linkindexnode=9:input1linkindexoutput=0:input2linkindexnode=7:input2linkindexoutput=0:input4linkindexnode=10:input4linkindexoutput=0:
//#sm=2.0
//#blending=Alpha
//#ShaderName
Shader "ShaderFusion/CustomBlendingMode" {
	Properties {
...
//#DeferredVertexEnd
		...
		void surf (Input IN, inout SurfaceShaderFusion o) {
			...
			float alpha = 1.0;
//#PreFragBody
float4 node5 = tex2D(_Texture1,IN.sfuv1.xy);
float4 __sceneColor_node6 = (tex2D(_GrabTexture,((IN.screenPos.xy/IN.screenPos.w)+(((0,0))*(IN.screenPos.z*_GrabTexture_TexelSize.xy)))));
float node10 = ((LinearEyeDepth(tex2Dproj(_CameraDepthTexture, UNITY_PROJ_COORD(IN.screenPos)).r)-IN.screenPos.z)/1);
//#FragBody
alpha = (node10);
specular = (float4(0,0,0,1));
emissive = (((node5) * (__sceneColor_node6)) * 2);
